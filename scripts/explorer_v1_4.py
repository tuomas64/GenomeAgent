...
def main() -> None:
    ...
    try:
        ...
        # Always save remote diagnostics before deciding whether the scan failed.
        # GNU find commonly returns status 1 when a directory is unreadable or
        # when a file disappears during a long live-filesystem traversal. Those
        # conditions do not invalidate the successfully streamed inventory.
        ...
        if return_code == 1:
            print(
                "WARNING: remote find completed with status 1. "
                "The inventory will be retained; see the diagnostic files.",
                flush=True,
            )
        elif return_code != 0:
            raise RuntimeError(
                f"Remote find exited with status {return_code}. "
                f"See {run_dir / 'permission_errors.txt'} and "
                f"{run_dir / 'other_find_messages.txt'}."
            )
    ...
    result = {
        "explorer": "GenomeAgent Explorer v1.4",
        ...
        "remote_find_exit_status": return_code,
        "completed_with_warnings": return_code == 1,
    ...
    print()
    print("=" * 68)
    print("GenomeAgent Explorer v1.4 complete")
    print("=" * 68)
    ...
    print(f"Find exit status    : {return_code}")
    print(f"Completed warnings  : {'yes' if return_code == 1 else 'no'}")
