...
echo "GenomeAgent Build v0.5 - Explorer Core"
echo "Creating read-only observation layer..."
echo

mkdir -p genomeagent observations docs

cat > genomeagent/explorer_core.py << 'PY'
#!/usr/bin/env python3
...
from genomeagent.explorer_core import main
...
git add .
git commit -m "GenomeAgent v0.5: Add read-only Explorer Core" || true

echo
echo "Build v0.5 complete."
echo
echo "Try:"
echo "  python explorer.py"
echo "  ls observations"
echo "  cat observations/*workspace_context*.json"
echo
echo "Then push:"
echo "  git push"
